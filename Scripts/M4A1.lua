...
This gun was designed to be used on a 3rd person based view.

Sight = 0 = Flip-up Iron Sight
Sight = 1 = Red Dot
Sight = 2 = EOTech Holographic
Sight = 3 = AimPoint M4 Red Dot Sight

Magazines:
"STANAG" = 30 Round STANAG
"Beta-C" = 100 Round Beta-C. (Has a better chance of being jammed when auto firing)

Stocks: 
0 = None 
1 = Folding 
2 = Standard Telescoping.

Recievers: 
0 = Standard
1 = CQB.

All are able to be toggled below.

Damage:
This weapon does:
x10 damage to headshots
x2 damage to torso shots
and normal damage to limbs (When fired at legs, it has a chance of completely removing them)
(Stock damage (unless edited) is 35) 

Weapon Keys:
R = Reload
C = Re-clock (Remove one round from the barrel) the gun after being Jammed (If it doesnt fire, its most likely jammed)
E = Equip Silencer (When silenced, the weapon becomes much silencer and much more accurate against targets)
F = Toggle laser sight / badly done flashlight
U = Toggle fire modes
Y = Zoom into where your cursor is. If its over a living thing, it'll literally watch that unit and you can pick he/she off


Extra Information:
The M4A1 has an exremely high recoil and shouldnt be used on automatic unless you're doing an assault
I reccomend using burst fire or single fire for medium or long distances

To Do:
Fix up walking after the gun is loaded
Fix up a few bugs with the gun breaking up if you use another gun.
Fix up animation bugs
--]]
...
DC = BrickColor.new("Really white")
...
-- Stats/Abilities
selected = false --Is it selected from when executed?
canDual = false --Can it be dual selected?
dual = false -- Dual from the start
Button1Down = false 
damage = 45 -- Damage 
canFire = true -- Can it be shot or is it there for aesthetics?
canFire2 = false --Testing
readyTime = 0.11 -- Time it takes for it to lock and load
automatic = true -- Automatic from start?
burst = false -- burst fire from start?
burstCount = 0 -- Start burst count
burstCountMax = 3 -- max bursts
canSilence = true -- can be silenced? (Using E key)
silenced = true -- Silenced from start?
canZoom = true -- can zoom into an enemey's POV (really op)
zoom = false -- zoom from start
switchToSingle = true
switchToBurst = true
